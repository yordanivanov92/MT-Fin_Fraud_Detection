...
the dataset variations. In Tables 3 and 4, we can observe ROC AUC and PR
AUC of the models that were used in this study.

\begin{table}

\caption{\label{tab:ucsd_models_AUC}AUC Metric Model Variations}
\centering
...
\caption{\label{tab:ucsd_model_PR}PR Metric Model Variations}
\centering
\resizebox{\textwidth}{!}{\begin{tabular}[t]{lrrrrrrr}
\toprule
type & original & original\_radial & weighted & weighted\_radial & down & up & SMOTE\\
...
The Logistic Regression performs relatively competitive and having a
huge computational speed advantage means it is in no way irrelevant.
