...
\usepackage[top=1in, bottom=1in, left=1.18in, right=1in]{geometry}
\linespread{1.5}
\usepackage{hyperref}
...
line that is close to the 45-degree separator means that it performs no
better than simple random guessing.

\includegraphics{figures/roc_curve_example.png}\{width = 60\%\}
...
will still be high (and close to 100\% in some cases).
...
\begin{itemize}
\item
  State if data-sampling techniques have made any real improvement - put
  the ROC, PR and AUC measures in appendix or in small table, graphs.
\item
  Get the best classifier from each model type and plot all of them -
  ROC, PR and AUC
\item
  Discuss result
\end{itemize}
