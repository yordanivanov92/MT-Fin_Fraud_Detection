...
2016). ML focuses on the problem of learning, that can be phrased as the
problem of gathering knowledge through experience (Dal Pozzolo and
Bontempi 2015). The process can generally be defined as observing an
event, formulating a hypthesis and then make predictions, or shorter -
...
often than represent just a small fraction of all transactions, i.e.~the
problem of imbalanced classes (Dal Pozzolo et al. 2014). Unfortunately,
most ML models are not developed to explicitly deal with this type of
data (He and Garcia 2009). Another difficulty that has halted the
evolution of FFD techniques is the lack of publicly available data
(Lopez-Rojas and Axelsson 2014). Thus, in this paper we attempt to
counter both of these problems.

The objective of this paper is to give an extensive overview of
different ML techniques to battle with the problem of FFD, work with
three different publicly available datasets and in the end discuss which
methods show the strongest performances. The ML techniques that are to
be evaluated are Artificial Neural Networks, Support Vector Machines,
Gradient Boosting Machines, eXtreme Gradient Boosting, Random Forest and
Logistic Regression. Moreover, each model performance will be tested on
variations of the three original datasets, as different data-sampling
will be applied in attempt to minimize the effect of the class
imbalance.
...
when working with dataset that have imbalanced classes.
