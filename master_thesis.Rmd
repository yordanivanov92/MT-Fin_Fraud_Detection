...
Let us define our binary classification dataset as ${\{(x_1,y_1),(x_2,y_2),...,(x_p,y_p)\}}$, where $x_i\in \mathbb{R}^n$ represents an n-dimensional data point and $y_i \in \{-1,1\}$ represents the label of the class of that data point, $i = 1,...,p$. Let $X$ represent the vector of features and $Y$ the vector of dependent variables.   
...
$$f(x) = sign(w \cdot \Phi(x) + b) = sign(\sum_{i=1}^{p}{\alpha_iy_i}K(x_i,x_j) + b) $$
...
$$Linear: \;\;\; K(x_i,x_j) = \sum_{k=1}^{l}x_{ik}x_{jk}  $$
$$Radial: \;\;\; K(x_i,x_j) = exp(-\gamma\sum_{k=1}^{l}(x_{ik}-x{jk})^2) $$
...
1. For $b$ = 1 to $B$ ($B$ is representing the number of bootstrapped datasets):
    ...
    + Grow a RF tree $T_b$ on the previously bootstrapped dataset by recursively looping the below given steps for each tree terminal node, up until the $n_min$ node (minimum size node) is reached:
...
Due to its flexibility and ease of implementation, the GBM algorithm has proven to be a successful model, that offers high predictive power when dealing with machine learning problems [@whiting2012machine, @johnson2014learning]. Furthermore, when including a random element in the algorithm, as in the Stochastic GBM (SGBM), results tend to improve [@friedman2002stochastic]. We describe the general form of the SGBM for a two-class problem below.

_Stochastic Gradient Boosting Machines (Stochastic GBM)_

Two-Class Classification as in the **gbm** R package.

1. Initialize $f_{0}(x) = \arg\min_{\rho}\sum_{i=1}^{N}\Psi(y_i,\rho)$ and set $g = \{1,2\}$ for the two classes.
...
2. For $m=1$ to $M$ do:
    ...
    + For $g=1$ to 2 do:
        ...
        - Compute negative gradient:
          $$z_{ig}=-\frac{\partial{\Psi(y_{ig},f_{g}(\mathbf{x}_i))}}{\partial{f_{g}(\mathbf{x}_i)}}\rvert_{f_{g}(\mathbf{x}_i)=\widehat{f_{g}}(\mathbf{x}_i)},\; i=1,...,N$$
        ...
        - Compute the optimal terminal node predictions, $\rho_{1g},...,\rho_{Kg}$, as:
          $$\rho_{kg}=\arg\min_{\rho}\sum_{\mathbf{x}_i \in S_k} \Psi(y_i,\widehat{f_{g}}(\mathbf{x}_{i}+\rho))$$,
