...
kable(ucsd_auc_table, format = "pandoc", digits = 3, caption = "AUC Metric Model Variations")
```



```{r ucsd_models_PR, echo = FALSE}
library(dplyr)
library(knitr)
library(kableExtra)
load(paste0(getwd(),"/figures/ucsd/all_graphs_tables.RData"))
kable(ucsd_PR_table, format = "pandoc", digits = 4, caption = "PR Metric Model Variations") 
```

It is quite evident from these two tables that the best performing model throughout each setting is Random Forests. Unsurprisingly it is followed by the eXtreme Gradient Boosting, which also produces really good results. The SVM is also expectedly suffering, due to the severe class imbalance, which as mentioned in Section 3, leads to the support vector being pushed close to the majority class, thus ignoring the minority. 

In Figure ... we also visually show the differences between the different Machine Learning Techniques. 

![ROC and PR Curves](figures/ucsd/ucsd_PR_ROC.png){width=100%}
