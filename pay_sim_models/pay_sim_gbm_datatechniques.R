...
cluster <- makeCluster(detectCores() - 2) 
registerDoParallel(cluster)
paySim_gbm_weighted_fit <- train(isFraud ~ .,
                                     data = paySim_train,
                                     method = "gbm",
                                     verbose = FALSE,
...
gbm_weight_imp <- varImp(paySim_gbm_weighted_fit, scale = FALSE)
plot(gbm_weight_imp)
...
cluster <- makeCluster(detectCores() - 2) 
registerDoParallel(cluster)
paySim_gbm_down_fit <- train(isFraud ~ .,
                                 data = paySim_train,
                                 method = "gbm",
                                 verbose = FALSE,
                                 metric = "ROC",
...
gbm_down_imp <- varImp(paySim_gbm_down_fit, scale = FALSE)
plot(gbm_down_imp)
...
registerDoParallel(cluster)
paySim_gbm_up_fit <- train(isFraud ~ .,
                               data = paySim_train,
                               method = "gbm",
                               verbose = FALSE,
                               metric = "ROC",
...
gbm_up_imp <- varImp(paySim_gbm_up_fit, scale = FALSE)
plot(gbm_up_imp)
...
cluster <- makeCluster(detectCores() - 2) 
registerDoParallel(cluster)
paySim_gbm_smote_fit <- train(isFraud ~ .,
                                  data = paySim_train,
                                  method = "gbm",
                                  verbose = FALSE,
                                  metric = "ROC",
...
gbm_smote_imp <- varImp(paySim_gbm_smote_fit, scale = FALSE)
plot(gbm_smote_imp)
...
paySim_gbm_model_list <- list(original = paySim_gbm,
                              weighted = paySim_gbm_weighted_fit,
                              down = paySim_gbm_down_fit,
                              up = paySim_gbm_up_fit,
                              SMOTE = paySim_gbm_smote_fit)


paySim_gbm_model_list_roc <- paySim_gbm_model_list %>%
  map(paySim_test_roc, data = paySim_test)

paySim_auc_gbm <- as.data.frame(paySim_gbm_model_list_roc %>% map(auc))
saveRDS(paySim_auc_gbm, 
        file = paste0(getwd(),"/figures/paysim/gbm/paySim_auc_gbm.rds"))

paySim_gbm_results_list_roc <- list(NA)
num_mod <- 1

for(the_roc in paySim_gbm_model_list_roc){
  paySim_gbm_results_list_roc[[num_mod]] <-
    data_frame(tpr = the_roc$sensitivities,
               fpr = 1 - the_roc$specificities,
               model = names(paySim_gbm_model_list)[num_mod])
  num_mod <- num_mod + 1
}

paySim_gbm_results_df_roc <- bind_rows(paySim_gbm_results_list_roc)
saveRDS(paySim_gbm_results_df_roc, 
        file = paste0(getwd(),"/figures/paysim/gbm/paySim_gbm_results_df_roc.rds"))

custom_col <- c("#000000", "#009E73", "#0072B2", "#D55e00", "#CC79A7")

ggplot(aes(x = fpr, y = tpr, group = model), data = paySim_gbm_results_df_roc) +
...
paySim_gbm_calc_auprc <- function(model, data) {
...
paySim_gbm_model_list_pr <- paySim_gbm_model_list %>%
  map(paySim_gbm_calc_auprc, data = paySim_test)

# Precision recall Curve AUC calculation
paySim_PR_gbm <- as.data.frame(paySim_gbm_model_list_pr %>% map(function(the_mod) the_mod$auc.integral))
saveRDS(paySim_PR_gbm, 
        file = paste0(getwd(),"/figures/paysim/gbm/paySim_PR_gbm.rds"))

paySim_gbm_results_list_pr <- list(NA)
num_mod <- 1
for (the_pr in paySim_gbm_model_list_pr) {
  paySim_gbm_results_list_pr[[num_mod]] <-
    data_frame(recall = the_pr$curve[, 1],
               precision = the_pr$curve[, 2],
               model = names(paySim_gbm_model_list_pr)[num_mod])
  num_mod <- num_mod + 1
}

paySim_gbm_results_df_pr <- bind_rows(paySim_gbm_results_list_pr)
saveRDS(paySim_gbm_results_df_pr, 
        file = paste0(getwd(),"/figures/paysim/gbm/paySim_gbm_results_df_pr.rds"))
